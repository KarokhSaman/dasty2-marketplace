    ...
    <div className="max-w-lg mx-auto pb-8">
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-white rounded-2xl border border-gray-100 p-5 mb-2 space-y-3">
          <h2 className="text-sm font-bold text-gray-800 mb-1">{m.accountUpdateTitle()}</h2>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">{m.registerNameLabel()}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">{m.loginPhoneLabel()}</label>
            <input value={phone} onChange={e => setPhone(e.target.value)} required dir="ltr"
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">{m.registerCityLabel()}</label>
            <CustomSelect
              value={city}
              onChange={setCity}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">{m.registerAddressLabel()} <span className="text-rose-500">*</span></label>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <button onClick={() => setShowHow(v => !v)}
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400 mt-0.5">{m.acctStepByStep()}</p>
          </div>
        ...
        <button onClick={() => setShowFees(v => !v)}
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-gray-800">{m.acctServiceFeesSec()}</p>
        ...
        {showFees && (
          <div className="px-4 pb-4 bg-gray-50">
            ...
            <p className="text-xs text-gray-400 mt-2 text-center">{m.acctSellerFeesNote()}</p>
          </div>
        )}
