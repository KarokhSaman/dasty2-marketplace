...
export default function ProductDetailPage() {
  ...
  // Live query resolved to null → item not approved / deleted. Overrides any stale seed.
  if (live === null) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-gray-400">
        <p className="text-xl font-medium mb-2">{m.notFound()}</p>
        <button onClick={() => router.history.back()} className="text-sm text-rose-500 hover:underline mt-2">
          {m.back()}
        </button>
      </div>
    );
  ...
  // No seed and live still loading → shimmer (direct URL / refresh / new tab).
  if (!product) {
    return (
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-12">
        ...
        <div>
          <div className="flex gap-2 mb-3 flex-wrap">
            ...
            {product.featured && (
              <span className="text-xs px-3 py-1 rounded-full bg-rose-100 text-rose-600 font-medium">{m.featured()}</span>
            )}
          </div>
          ...
          {product.seq && (
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs text-gray-400">{m.productCode()}:</span>
