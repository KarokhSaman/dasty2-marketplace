...
function CityDropdown({ city, setCity, cities, locale }) {
  ...
  const label = city === "all" ? m.allCities() : (getCityLabel(city, locale) ?? city);
  const options = [
...
const HOME_FEATURED_KEY = "dasty2-home-featured-cache";
const HOME_RESULTS_KEY  = "dasty2-home-results-cache";
...
export default function HomePage() {
  ...
  // Cached product arrays from the previous visit — used to paint instantly
  // on remount so we don't flash skeletons while Convex re-resolves.
  const [cachedFeatured] = useState(() => readCache(HOME_FEATURED_KEY));
  const [cachedResults]  = useState(() => readCache(HOME_RESULTS_KEY));
  ...
  // Use cached items while the first page is still loading on remount
  ...
  // Seed the in-memory product cache so detail page can open instantly on tap.
  useEffect(() => {
  ...
  function saveState() {
    ...
    try {
      sessionStorage.setItem(HOME_FEATURED_KEY, JSON.stringify(featuredProducts));
      sessionStorage.setItem(HOME_RESULTS_KEY,  JSON.stringify(results));
    } catch {
    ...
    <div>
      ...
      {!isLoading && (
        <div className="flex items-center justify-between gap-2 mb-4">
          <CityDropdown city={city} setCity={setCity} cities={availableCities} locale={locale} />
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-400 whitespace-nowrap">{m.sortBy()}</span>
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-gray-300">
          ...
          <p className="text-lg font-medium text-gray-400">{m.noProducts()}</p>
          <p className="text-sm mt-1 text-gray-300">{m.tryDifferent()}</p>
        </div>
      )}
      ...
      {status === "Exhausted" && results.length > PAGE_SIZE && (
        <p className="text-center text-xs text-gray-300 py-6">— {m.noProducts()} —</p>
      )}
    </div>
  );
}
