    ...
    <div className="min-h-screen flex flex-col">
      ...
      {!isAddProduct && (
        ...
        <div className={`flex items-center justify-center transition-all duration-300 ${isNavVisible ? "gap-8 py-3 px-6" : "gap-4 py-2 px-4"} pb-[max(0.25rem,env(safe-area-inset-bottom))]`}>
          <Link to="/" onClick={handleHomeTap} className={`inline-flex flex-col items-center transition-all duration-300 ${isNavVisible ? "gap-0.5 px-3 py-1.5" : "gap-0 px-2 py-1"} rounded-lg border-b-2 ${isHomeTab ? "text-[var(--color-ember-600)] border-[var(--color-ember-600)]" : "text-[var(--color-ink-soft)] hover:text-[var(--color-ink)] border-transparent"}`}>
