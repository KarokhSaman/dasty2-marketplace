        ...
        <div>
          ...
          <div className="relative">
            <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder={t.fieldPricePlaceholder} min={0} dir="ltr"
              className={`w-full border rounded-xl px-4 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 ${errors.price ? "border-red-400 bg-red-50" : "border-gray-200"}`} />
            {price && (
              <button type="button" onClick={() => setPrice("")}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-400 hover:text-gray-600 transition-colors">
